...
class ChangeLogDisplay:
    ...
    def __init__(
        ...
        self.portfolio_changelog = changelog
        self.reporter = reporter or ConsoleReporter()

    def display(self) -> None:
        """Print session change log, summary stats, and portfolio evolution."""
        self.reporter.header("📜 SESSION CHANGE LOG")
        print()

        if (
            not self.portfolio_changelog
            or self.portfolio_changelog.get_number_of_snapshots() == 0
        ):
            self.reporter.warning("No changes recorded in this session.")
            ...
            print(
                ...
                f"{self.portfolio_changelog.get_number_of_snapshots()}",
            )
            print()
            ...
            for entry in self.portfolio_changelog.get_all_portfolio_snapshots():
            ...
            action_counts = self.portfolio_changelog.get_action_counts(
            ...
            total_delta_impact = self.portfolio_changelog.get_total_delta_impact()
            print(f"\n  Net Delta Impact: {total_delta_impact:+.1f}")

            # Portfolio evolution
            if self.portfolio_changelog.get_number_of_snapshots() > 0:
                first_snapshot = self.portfolio_changelog.get_all_portfolio_snapshots()[
                    0
                ]["portfolio_snapshot"]
                last_snapshot = self.portfolio_changelog.get_all_portfolio_snapshots()[
                    -1
                ]["portfolio_snapshot"]
