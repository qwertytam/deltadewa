...
class PositionDetailDisplay:
    ...
    def display(self) -> None:
        ...
        if not df_positions.empty:
            ...
            def _fmt_currency(x: Any) -> str:  # ruff: ignore[any-type]  # pandas style formatter; float/int(x) requires Any — object would fail mypy
                try:
                    return f"${float(x):,.2f}"
                except Exception:  # pylint: disable=broad-except
                    return str(x)

            def _fmt_pct(x: Any) -> str:  # ruff: ignore[any-type]  # pandas style formatter; float/int(x) requires Any — object would fail mypy
                try:
                    return f"{float(x):.1%}"
                except Exception:  # pylint: disable=broad-except
                    return str(x)

            def _fmt_int(x: Any) -> str:  # ruff: ignore[any-type]  # pandas style formatter; float/int(x) requires Any — object would fail mypy
                try:
                    return f"{int(x):.0f}"
                except Exception:  # pylint: disable=broad-except
                    return str(x)

            def _fmt_greek(x: Any) -> str:  # ruff: ignore[any-type]  # pandas style formatter; float/int(x) requires Any — object would fail mypy
                try:
                    return f"{float(x):.5g}"
                except Exception:  # pylint: disable=broad-except
                    return str(x)
