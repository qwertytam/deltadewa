...
class OptionPortfolio:
    ...
    def analyze_roll(
        ...
        if position_index < 0 or position_index >= len(self.positions):
            raise IndexError(
                f"Position index {position_index} out of range (0-{len(self.positions)-1})"
            )
        ...
        comparison_df = pd.DataFrame(
            {"Current": current_metrics, "Proposed": proposed_metrics, "Delta": deltas}
        )
    ...
    def commit_roll(
        ...
        self.positions[position_index] = OptionPosition(
            new_option, quantity, contract_size=current_pos.contract_size, symbol=current_pos.symbol
        )
