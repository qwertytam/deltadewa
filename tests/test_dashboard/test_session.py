...
# #245: the real config/ips.yaml is gitignored, so start_session's own
# default path (also config/ips.yaml) is no longer guaranteed to exist on a
# fresh checkout. These tests exercise start_session's *behaviour*, not that
# particular default, so they point at the tracked example instead.
_EXAMPLE_IPS_PATH = (
    Path(__file__).parent.parent.parent / "config" / "ips.example.yaml"
)


def _start_session(**kwargs: object) -> SessionContext:
    """``start_session`` with ``ips_path`` defaulted to the tracked example."""
    kwargs.setdefault("ips_path", _EXAMPLE_IPS_PATH)
...
class TestStartSession:
    ...
    def test_loads_ips_config_by_default(self) -> None:
        ...
        ctx = _start_session(globals_dict={})
    ...
    def test_default_exercise_style_is_european_for_spx_symbol(self) -> None:
        ...
        ctx = _start_session(globals_dict={"portfolio": spx_portfolio})
    ...
    def test_role_is_stored_without_conditional_behaviour(self) -> None:
        ...
        combined_ctx = _start_session(globals_dict={}, role="combined")
        other_ctx = _start_session(globals_dict={}, role="hedge-only")
    ...
    def test_default_path_never_constructs_cboe_fred_provider(self) -> None:
        ...
        with patch.object(
            ...
            ctx = _start_session(globals_dict={}, use_live_market_data=False)
    ...
    def test_auto_load_default_false_starts_empty(self) -> None:
        ...
        ctx = _start_session(globals_dict={}, auto_load_default=False)
    ...
    def test_live_market_data_flag_constructs_cboe_fred_provider(
        ...
        with patch.object(
            ...
            ctx = _start_session(globals_dict={}, use_live_market_data=True)

        # The freshness window is IPS policy (market_environment.
        # data_ttl_minutes), not the provider's constructor default. The
        # tracked config/ips.example.yaml carries 1440 (24h; #245), not the
        # dataclass's own 15-minute default.
    ...
    def test_loads_dashboard_config_when_present(self, tmp_path: Path) -> None:
        ...
        ctx = _start_session(globals_dict={}, dashboard_path=config_path)
